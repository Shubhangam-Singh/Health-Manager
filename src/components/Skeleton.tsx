...
export function SkeletonLine({ w = "w-full" }: { w?: string }) {
  return <div className={`h-3.5 ${w} animate-pulse rounded bg-[var(--skeleton)]`} />;
}
    ...
    <div className="grid gap-4 sm:grid-cols-3">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="rounded-[10px] border border-[var(--border)] bg-[var(--surface)] p-5">
          <div className="h-8 w-14 animate-pulse rounded bg-[var(--skeleton)]" />
          <div className="mt-2 h-3 w-28 animate-pulse rounded bg-[var(--skeleton)]" />
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <div className="h-7 w-56 animate-pulse rounded bg-[var(--skeleton)]" />
        <div className="h-3.5 w-72 animate-pulse rounded bg-[var(--skeleton)]" />
      </div>
